<?php
    ...
    if (isset($_POST['search'])) {
        if (!empty($_POST['table_search'])) {
            ...
            $sql = "SELECT * FROM detailEntree WHERE Produit_id LIKE ? OR Entree_idE LIKE ? OR quantite LIKE ? OR prix LIKE ? "; // Requête de recherche
            ...
            if (count($results) > 0) {
                ...
                foreach ($results as $result) {
                    // Afficher les informations du detailEntree
                    echo $result['Produit_id'] . ', ' . $result['Entree_idE'] . ', ' . $result['quantite'] . ',' . $result['prix'] . '<br>';
                }
            } else {
                echo "Aucun detailEntree trouvé.";
            }
        } else {
            echo "Veuillez entrer un detailEntree à rechercher.";
        }
    }
?>